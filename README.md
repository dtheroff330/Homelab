...
| degoog | Mini PC | Self-hosted search aggregator |
...
- [04-mini-pc.md](04-mini-pc.md) - Docker Compose, Vaultwarden, degoog, Caddy
...
`Debian` `Linux` `Docker` `Docker Compose` `Caddy` `Tailscale` `Unbound` `AdGuard Home` `Prometheus` `Grafana` `Vaultwarden` `degoog` `SSH` `ufw` `fail2ban` `LUKS` `DNS` `DHCP` `Porkbun DNS-01` `ed25519` `OpenMediaVault`
